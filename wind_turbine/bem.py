...
try:
    from wind_turbine._bem_cpp import evaluate_rotor_cpp as _evaluate_rotor_cpp_impl
...
def _build_polar_grids(
    ...
    cd_grid = np.maximum(np.vstack(cd_rows), 1e-5).astype(float, copy=False)
    return re_bins.astype(float, copy=False), alpha_ref.astype(float, copy=False), cl_grid, cd_grid


def _evaluate_rotor_cpp(
    rotor: RotorConfig,
    polar_db: XfoilPolarDatabase,
    airfoil: str,
    blades: int,
...
) -> tuple[list[SectionResult], RotorPerformance] | None:
    ...
    try:
        out = _evaluate_rotor_cpp_impl(
            radius_m=float(rotor.radius_m),
            wind_speed_ms=float(rotor.wind_speed_ms),
            air_density=float(rotor.air_density),
            dynamic_viscosity=float(rotor.dynamic_viscosity),
            pitch_deg=float(rotor.pitch_deg),
            blades=int(blades),
            tip_speed_ratio=float(tip_speed_ratio),
            hub_radius_ratio=float(hub_radius_ratio),
            r_m=r_m,
            chord_m=chord_m,
            twist_deg=twist_deg,
            re_bins=re_bins,
    ...
    for i, sec in enumerate(sections):
        section_results.append(
            SectionResult(
                ...
                cd=float(cd[i]),
                a=float(a[i]),
                a_prime=float(a_prime[i]),
                cn=float(cn[i]),
                ct=float(ct[i]),
                dthrust_n=float(dthrust_n[i]),
                dtorque_nm=float(dtorque_nm[i]),
                local_solidity=float(local_solidity[i]),
            )
        )
    ...
    perf = RotorPerformance(
        cp=float(out["cp"]),
        ct=float(out["ct"]),
        power_w=float(out["power_w"]),
        thrust_n=float(out["thrust_n"]),
        torque_nm=float(out["torque_nm"]),
        root_moment_nm=float(out["root_moment_nm"]),
        solidity_mean=float(out["solidity_mean"]),
    )
...
def evaluate_rotor(
    ...
    cpp_result = _evaluate_rotor_cpp(
        rotor=rotor,
        polar_db=polar_db,
        airfoil=airfoil,
        blades=blades,
