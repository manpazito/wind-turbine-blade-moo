...
class DummyPolarsWithTables:
    ...
    def __init__(self):
        ...
        for re in self.reynolds_bins:
            ...
            self._tables[re] = pd.DataFrame({'alpha': self._alpha, 'cl': cl, 'cd': cd})
    ...
    def sample(self, airfoil, reynolds, alpha_deg):
        ...
        return PolarPoint(cl=(1.0 - w) * cl_lo + w * cl_hi, cd=(1.0 - w) * cd_lo + w * cd_hi)

def test_cpp_and_python_bem_paths_match(monkeypatch):
    ...
    sections = bem.design_blade_geometry(rotor=rotor, design_space=design_space, polar_db=polars, airfoil='4412', blades=3, tip_speed_ratio=7.1, design_aoa_deg=6.0, hub_radius_ratio=0.2, chord_scale=1.0, twist_scale=1.0)
    cpp_sections, cpp_perf = bem.evaluate_rotor(rotor=rotor, polar_db=polars, airfoil='4412', blades=3, tip_speed_ratio=7.1, hub_radius_ratio=0.2, sections=sections)
    monkeypatch.setattr(bem, '_evaluate_rotor_cpp_impl', None)
    py_sections, py_perf = bem.evaluate_rotor(rotor=rotor, polar_db=polars, airfoil='4412', blades=3, tip_speed_ratio=7.1, hub_radius_ratio=0.2, sections=sections)
    assert len(cpp_sections) == len(py_sections)
    assert pytest.approx(py_perf.cp, rel=1e-07, abs=1e-09) == cpp_perf.cp
    assert pytest.approx(py_perf.ct, rel=1e-07, abs=1e-09) == cpp_perf.ct
    assert pytest.approx(py_perf.torque_nm, rel=1e-07, abs=1e-06) == cpp_perf.torque_nm
    assert pytest.approx(py_perf.thrust_n, rel=1e-07, abs=1e-06) == cpp_perf.thrust_n
    assert pytest.approx(py_perf.root_moment_nm, rel=1e-07, abs=1e-05) == cpp_perf.root_moment_nm
