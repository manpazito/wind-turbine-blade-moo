...
def test_surrogate_backend_generates_polars_and_coordinates(tmp_path):
    ...
    sample = db.sample('4412', reynolds=450000.0, alpha_deg=6.0)
...
def test_auto_backend_falls_back_when_executable_is_missing(tmp_path):
    cfg = XfoilConfig(executable='xfoil-this-command-does-not-exist', backend='auto', fallback_to_surrogate=True, reynolds_bins=[200000, 600000])
    with pytest.warns(RuntimeWarning, match='XFOIL executable was not found'):
        db = XfoilPolarDatabase(config=cfg, cache_dir=tmp_path)
    db.prepare(['2412'])
    sample = db.sample('2412', reynolds=350000.0, alpha_deg=4.0)
    assert sample.cd > 0.0
